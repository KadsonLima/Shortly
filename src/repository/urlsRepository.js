...
async function deleteDb(idUrl, token, userId) {
  const {
    rows: { [0]: url },
    rowCount: urlTokenVerify,
  } = await connection.query(`SELECT * FROM urls WHERE urls.id=$1 `, [idUrl]);
...
async function urlById(idUrl) {
  const {
    ...
    'SELECT u.id, u.url, u."shortUrl" FROM urls u WHERE id=$1',
    [idUrl]
  );
...
export { urlRanking, shortyUrl, getUrls, deleteDb, urlById, rankingUrls };
